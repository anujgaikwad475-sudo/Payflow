...
import { Wallet2, ArrowRight, User, Mail, Phone, Lock, AlertCircle } from 'lucide-react';
...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Signup failed. Check credentials and retry.');
    } finally {
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center px-4 py-10 overflow-hidden bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-black">
      <div className="absolute top-1/4 -right-20 w-80 h-80 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-1/4 -left-20 w-80 h-80 bg-teal-500/10 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="max-w-md w-full relative z-10">
        ...
        <div className="bg-slate-900/70 backdrop-blur-xl border border-slate-800 p-8 rounded-2xl shadow-2xl">
          <div className="text-center mb-6">
            <h2 className="text-2xl font-bold tracking-tight text-white">Create your Wallet</h2>
            <p className="text-sm text-slate-400 mt-1">Get started with immediate digital transfers</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              <span>{isSubmitting ? 'Creating...' : 'Register Wallet'}</span>
